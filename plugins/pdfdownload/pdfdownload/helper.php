...
class PaycartHelperPdfdownload extends PaycartHelper
{
	...
	function getResultForInvoices()
	{
		...
		if( empty($txnDateFrm) || empty($txnDateTo )){
			$txnDateFrm  = JRequest::getVar('pdfdownload_txnDateFrm','00-00-0000');
			$txnDateTo  = JRequest::getVar('pdfdownload_txnDateTo','00-00-0000');
			//if all fields are blank then return false
			if(empty($txnDateFrm) && empty($txnDateTo))
				return false;
			elseif (empty($txnDateFrm) && !empty($txnDateTo)){
		...
		if(JRequest::getVar('pdfdownload_deleteFiles',0)){
			$result['totalResult'] = $this->getTotalResults($txnDateFrm,$txnDateTo);
			$mysess->set('pdfdownload_total',$result['totalResult']);			
		}
		
		$result['result'] = $this->_getResultInvoices($txnDateFrm,$txnDateTo);
		return $result;
	}
	 ...
	 */
	function getTotalResults($txn_from , $txn_to)
	{	
...
/**
	 ...
	 */
	function _getResultInvoices($txnDateFrm,$txnDateTo)
	{
		$mysess  	= Rb_Factory::getSession();
		//need to ask this from users
		$limit 	    = PAYCART_PDF_EXPORT_LIMIT;
		$limitStart = $mysess->get('pdfdownload_start',0);
		$records    = $this->getInvoiceWithinDates($txnDateFrm , $txnDateTo, $limitStart, $limit);
		return $records;
	}
	
    function getInvoiceWithinDates($startDate, $endDate, $limitStart, $limit)
		{
			$query = new Rb_Query();
