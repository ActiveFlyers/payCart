...
{  
	...
	function addSorting($query,$sort)
	{
		switch ($sort)
		{
			case paycart::SORTING_OPTION_HITS         : $query->order('pp.`hits` DESC');
						 		  						break;

			case paycart::SORTING_OPTION_CREATED_DATE : $query->order('pp.`created_date` DESC');
								  						break;								
			
			case paycart::SORTING_OPTION_PRICE_LOW 	  : $query->order('pp.`price` ASC');
								  						break;
			
			case paycart::SORTING_OPTION_PRICE_HIGH   : $query->order('pp.`price` DESC');
								  						break;
		}
	...
	public function getAllAttributeOptionsByCategory($categoryId)
	{
		...
		return $query->select("`productattribute_id`,`productattribute_value` ,COUNT(av.`product_id`) as 'productCount'")
					 ...
					 ->innerJoin('`#__paycart_product` AS pp ON pp.productcategory_id = pc.productcategory_id and pp.published = 1 AND pp.visible = 1 and pp.product_id = av.product_id ')
					 ->group('`productattribute_id`,`productattribute_value`')
					 ->dbLoadQuery()
					 ->loadObjectList();
	}
