 ...
 * Cart Model
 */
class PaycartModelUser extends PaycartModel
{
	protected $recordId ;
    ...
    protected function _buildQueryFrom(Rb_Query &$query)
    {
    	...
    	$join = ' `#__paycart_user` AS t  ON ( t.`user_id` = joomlausertbl.`id` ) ';
    	
    	$query->_q1->select(' joomlausertbl.`id` AS user_id ')
    	...
    	while(!empty($cloneValue) && !empty($cloneOP)){
    		...
    		if(JString::strtoupper($op) != 'LIKE'){
				if(($key == 'usertype')){
					...
					$query->where("  user_id IN( SELECT map.`user_id` 
...
class PaycartModelformUser extends PaycartModelform { }