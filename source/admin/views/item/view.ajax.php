...
class PaycartAdminViewInvoice extends PaycartAdminBaseViewInvoice
{	
	public function email()
	{
		...
		if(!$this->get('confirmed')){
			$this->_setAjaxWinTitle(Rb_Text::_('COM_PAYCART_INVOICE_EMAIL_WINDOW_TITLE'));
			$this->_setAjaxWinBody(Rb_Text::_('COM_PAYCART_INVOICE_EMAIL_CONFIRM_MESSAGE'));
		
			$this->_addAjaxWinAction(Rb_Text::_('COM_PAYCART_CONFIRM'), 'paycart.admin.invoice.email.send('.$invoice_id.');', 'btn btn-info', 'id="paycart-invoice-email-confirm-button"');
			$this->_addAjaxWinAction(Rb_Text::_('COM_PAYCART_CLOSE'), 'rb.ui.dialog.close();', 'btn');
		...
		$email_controller 	= PaycartFactory::getInstance('email', 'controller', 'Paycartsite');
		...
		$email_view->assign('invoice', PaycartInvoice::getInstance($invoice_id)->toArray());
		...
		$url	= JUri::root().'index.php?option=com_paycart&view=invoice&invoice_id='.$invoice_id.'&key='.$key;
		$email_view->assign('pay_url', $url);
		
		// email content
		$body 	 = $email_view->loadTemplate('invoice');
		$subject = Rb_Text::_('COM_PAYCART_INVOICE_SEND_EMAIL_SUBJECT');
		$user 	 = PaycartFactory::getUser($rb_invoice['buyer_id']);		
		
		$result = $this->getHelper('utils')->sendEmail($user->email, $subject, $body);
		$msg = Rb_Text::_('COM_PAYCART_INVOICE_EMAIL_SENT');
		if(!$result){
			$msg = Rb_Text::_('COM_PAYCART_INVOICE_ERROR_SEND_ERROR');						
		}
